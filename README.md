...
The dataset is a periodically updated and expanded version of that published by Korábek et al. 2022, *Diversity* 14: 24 [https://doi.org/10.3390/d14010024](https://doi.org/10.3390/d14010024). The original data from that paper are available from [Dryad](https://doi.org/10.5061/dryad.pnvx0k6p5) **(NOTE: at present only a development version is available with the ORIGINAL dataset! Up-to-date data will be published in the second half of 2026.)**. The data consist of nucleotide sequences of partial mitochondrial genes obtained by PCR and assembled sequences of complete or nearly complete mitogenomes. These are data either obtained by me (O. Korábek) or downloaded from public databases (mostly [NCBI's Nucleotide database/GenBank](https://www.ncbi.nlm.nih.gov/nuccore), but also [BOLD](https://v4.boldsystems.org/). Only sequences accompanied by reasonably precise geographic location are included. Most of the data come from PCR targeting selected parts of the mitochondrial genome followed by Sanger sequencing. For many individuals, one or more fragments of the mitogenome are thus available with gaps of unknown length in between. Complete mitogenome sequences, mostly assembled from Illumina reads (WGS or RNAseq), are available for a small (but increasing) proportion of the sequenced individuals.
...
The sampling is far more sparse in the Near East and Northern Africa than in Europe. In terms of geographic bias in  the data, the most problematic species are *Helix salomonica* and *Levantina mahanica*, the eastern-most species. *Helix salomonica* lives in southeastern Turkey, Iraqi Kurdistan, and in western Iran up to the Lorestan Province of Iran; *Levantina mahanica* is distributed from west of Lake Urmia and the Iraqi Kurdistan to the Ilam province of Iran. These two species are broadly distributed, but the distribution of their mitochondrial diversity remains undocumented. The genus *Levantina* may be quite broadly distributed in the mountains of western Arabia, but data on distribution (and therefore also sequences) are lacking. For the African *Helix melanostoma*, which also has a relatively large range, samples are lacking from the western half of its range (extending up to Oran in Algeria). Only scarce data are available for the even more broadly distributed African *Helix pronuba* (lives from southern Tunisia to the Nile delta). The sampling of the group including *Helix cincta*, *Helix anctostoma*, and *Helix valentini* is strongly biased in favour of populations outside the native range and *H. anctostoma* was never sampled from its type locality in the Belen Pass in the Turkish province of Hatay. The main distribution ranges of typical *H. cincta* and the form known as *H. valentini* in the Levant, where the group originated, could now be traced thanks to photos posted on iNaturalist to the northwest of the Latakia Governorate (*H. cincta*) and to the east and south of the Latakia Governorate and the Tartus Governorate (*H. valentini*) of Syria. However, no sequences originate from there. *Helix cincta* was mainly sampled from likely or clearly anthropogenic populations from Cyprus, in western Anatolia and in Italy, Slovenia and Croatia.
...
Each individual is assigned a unique ID (8 characters, eg. "HE003187"). This ID appears at the beginning of the fasta header of each sequence. It may be followed by another identifier separated by "_" (isolate lab code for original data, GenBank accession number for downloaded data - but accession of one gene is usually used for all genes from the same individual - and "barcode" if from a barcoding dataset not available from GenBank), species name (may be obsolete, the names are not consistently updated in the fasta files), country (two-letter ISO code), and locality name (for internal use, not standardized). The ID stands for an individual snail, so it is identical between alignment files where the sequences come from the same snail. The ID provides a link between the sequences and the metadata.
#### How are the sequence data structured?
Aligning the sequences takes some effort, especially for the rRNA genes and non-coding sequences, and it is much easier to be done peacemeal. Moreover, from most individuals only short gene fragments were available. Therefore the data are stored in FASTA alignment files separately for individual protein-coding and rRNA genes and for each interval between these (these intervals largely consist of tRNA genes). That means that complete mitogenome sequences are also split into the individual alignment files. The intervals between PCGs and rRNAs are only kept for a given individual if there is a complete sequence spanning the region between two rRNA or protein-coding genes. That ensures some anchor for the sequence ends here, since there are often some non-conserved nucleotides between genes or at ends of the tRNAs. There is, however, an exception to this definition of the individual alignment files. Due to the position of primer used to amplify a fragment of *rrnS* (12S rRNA, *rrnS*) within *trnM*, the *rrnS* gene is kept in one file with *trnM* and *trnM* is not required to be complete if part of the *rrnS* fragment amplified by PCR.
...
### The files
Only the name stem is given here, which is preceded by version in the actual file names.
...
**Helicini_12S_TRNM**: *rrnS* and *trnM*, mostly incomplete; the end position of the *rrnS* gene (5' end of the sequence here) is uncertain and therefore the alignment file may not coincide with the gene limits; because the *rrnS* amplicon used in PCR-based studies extends into *trnM*, *trnM* is kept either up to the primer or, if complete, up to the 5' end of *nd3*.
...
Korábek O, Hausdorf B. (2026). Multilocus phylogeny of the land snail *Helix* differs substantially from the mitochondrial gene tree. *Molecular Phylogenetics and Evolution* 216: 108516. https://doi.org/10.1016/j.ympev.2025.108516
