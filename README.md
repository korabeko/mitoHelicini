...
shells_deposited_in	acronym of the institution or collection, where are deposited the shells (include any shells of the same species collected at the same site on the same occasion, not only the sequenced individual); collection acronyms are listed on a separate sheet “collection_acronyms”
collection_number	catalogue number, under which the shells are registered in the collection
isolate_code	isolate codes (lab codes used to identify the individual and DNA extracts made from it)
tissue_DNA_deposited_in	collection, where the analysed tissue sample is deposited (see the next column description for details)
tissue_DNA_collection_number	catalogue number, under which the tissue sample is registered in the collection. * Many samples added since 2021 were catalogued right away, while many of the tissue samples from studies published by Korábek et al. in 2014-2022 were deposited in ZMH in 2022. However, in most of the material collected until 2015 (and some of that collected later), the labelling and records were unsatisfactory and it was no longer possible to link tissue samples to DNA extracts and sequences, only to sampling sites. Catalogue numbers labeled with * mean that the link between sample and sequence data was not available, and the situation may be further complicated if 1) no tissue is left of the analysed individual and 2) sample from one individual was split into more tubes. On 29.10.2017 the system for databasing the samples was changed, and the link between individual and DNA isolate might have also been lost on this occasion.
16S_acc_nos	GenBank accession for the 16S rRNA gene
COX1_acc_nos	GenBank accession for the COX1 gene
COX2_acc_nos	GenBank accession for the COX2 gene
12S_acc_nos	GenBank accession for the 12S rRNA gene
...
SRA	semicolon-separated Sequence Read Archive accessions with details of the project stated (specified in [] as ddRAD, WGS, RNAseq, etc.)
genome_acc_nos	
...
Giusti F, Fiorentino V, Manganelli G. (2015). A neotype for Helix cincta Müller, 1774 (Gastropoda, Pulmonata, Helicidae). Journal of Conchology 42: 209–212.	n/a
